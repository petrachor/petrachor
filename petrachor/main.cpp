...
//		<< "    -o,--mode <full/peer>  Start a full node or a peer node (default: full).\n\n"
		...
		<< "Client transacting:\n"
		<< "    --ask <sand>  Set the minimum ask gas price under which no transaction will be mined (default " << toString(DefaultGasPrice) << " ).\n"
		<< "    --bid <sand>  Set the bid gas price to pay for transactions (default " << toString(DefaultGasPrice) << " ).\n"
...
//	MinerCLI m(MinerCLI::OperationMode::None);
    ...
    if (b.size())
    {
        ...
        {
            RLP config(b);
            author = author == Address() ? config[1].toHash<Address>() : author;
        }
        catch (...) {}
    }
			...
			if (!keyManager.load(masterPassword) && masterSet)
			{
				while (true)
				{
					...
					if (keyManager.load(masterPassword))
						break;
                    cout << "The password you entered is incorrect. If you have forgotten your password, and you wish to start afresh, manually remove the file: " << (getDataDir() / fs::path("keys.info")).string() << "\n";
				}
			}
		}
		else
