...
{
    KeyPair<ECDSA> init(ECDSA::Secret(sha3("initiator")));
    KeyPair<ECDSA> initR(ECDSA::Secret(sha3("initiator-random")));
	h256 initNonce(sha3("initiator-nonce"));
	
    KeyPair<ECDSA> recv(ECDSA::Secret(sha3("remote-recv")));
    KeyPair<ECDSA> recvR(ECDSA::Secret(sha3("remote-recv-random")));
    ...
    ECDSA::Secret ephemeralShared;
	BOOST_CHECK(crypto::ecdh::agree(initR.secret(), recvR.pub(), ephemeralShared));
    ECDSA::Secret expected(fromHex("20d82c1092f351dc217bd66fa183e801234af14ead40423b6ee25112201c6e5a"));
...
{
    KeyPair<ECDSA> init(ECDSA::Secret(fromHex("0x5e173f6ac3c669587538e7727cf19b782a4f2fda07c1eaa662c593e5e85e3051")));
    KeyPair<ECDSA> initR(ECDSA::Secret(fromHex("0x19c2185f4f40634926ebed3af09070ca9e029f2edd5fae6253074896205f5f6c")));
	h256 initNonce(fromHex("0xcd26fecb93657d1cd9e9eaf4f8be720b56dd1d39f190c4e1c6b7ec66f077bb11"));

    KeyPair<ECDSA> recv(ECDSA::Secret(fromHex("0xc45f950382d542169ea207959ee0220ec1491755abe405cd7498d6b16adb6df8")));
    KeyPair<ECDSA> recvR(ECDSA::Secret(fromHex("0xd25688cf0ab10afa1a0e2dba7853ed5f1e5bf1c631757ed4e103b593ff3f5620")));
    ...
    ECDSA::Secret ephemeralShared;
	BOOST_CHECK(crypto::ecdh::agree(initR.secret(), recvR.pub(), ephemeralShared));
    ECDSA::Secret expected(fromHex("0xe3f407f83fc012470c26a93fdff534100f2c6f736439ce0ca90e9914f7d1c381"));
	BOOST_REQUIRE(expected == ephemeralShared);
...
BOOST_AUTO_TEST_CASE(ecies_interop_test_primitives)
{
    ...
    KeyPair<ECDSA> k(ECDSA::Secret(fromHex("0x332143e9629eedff7d142d741f896258f5a1bfab54dab2121d3ec5000093d74b")));
    ECDSA::Public p(fromHex("0xf0d2b97981bd0d415a843b5dfe8ab77a30300daab3658c578f2340308a2da1a07f0821367332598b6aa4e180a41e92f4ebbae3518da847f0b1c0bbfe20bcf4e1"));
    ECDSA::Secret agreeExpected(fromHex("0xee1418607c2fcfb57fda40380e885a707f49000a5dda056d828b7d9bd1f29a08"));
    ECDSA::Secret agreeTest;
	BOOST_CHECK(crypto::ecdh::agree(k.secret(), p, agreeTest));
	BOOST_REQUIRE(agreeExpected == agreeTest);
	
    KeyPair<ECDSA> kmK(ECDSA::Secret(fromHex("0x57baf2c62005ddec64c357d96183ebc90bf9100583280e848aa31d683cad73cb")));
    ...
    KeyPair<ECDSA> kenc(ECDSA::Secret(fromHex("0x472413e97f1fd58d84e28a559479e6b6902d2e8a0cee672ef38a3a35d263886b")));
    ECDSA::Public penc(ECDSA::Public(fromHex("0x7a2aa2951282279dc1171549a7112b07c38c0d97c0fe2c0ae6c4588ba15be74a04efc4f7da443f6d61f68a9279bc82b73e0cc8d090048e9f87e838ae65dd8d4c")));
...
{
    auto localEph = KeyPair<ECDSA>::create();
    ECDSA::Secret localNonce = crypto::Nonce::get();
    auto remoteEph = KeyPair<ECDSA>::create();
    ECDSA::Secret remoteNonce = crypto::Nonce::get();
	bytes ackCipher{0};
	bytes authCipher{1};
