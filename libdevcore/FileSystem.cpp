...
const std::string defaultDataDirName = "petrichor";

std::string dev::getDefaultDataDirName() { return defaultDataDirName; }
...
{
    fs::path dataDir = (s_ethereumDatadir.empty() ? getDefaultDataDir() : s_ethereumDatadir);
    return _prefix.empty() ? dataDir : (dataDir / _prefix);
}

fs::path dev::getDefaultDataDir()
{
#if defined(_WIN32)
    _prefix[0] = toupper(_prefix[0]);
    char path[1024] = "";
    if (SHGetSpecialFolderPathA(NULL, path, CSIDL_APPDATA, true))
        return fs::path(path) / defaultDataDirName;
    else
    {
    ...
    return dataDirPath / ("." + defaultDataDirName);
#endif
}
