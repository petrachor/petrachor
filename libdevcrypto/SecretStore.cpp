...
{
	h128 r = h128::random();
    EncryptedKey key{encrypt(_s.ref(), _pass), toUUID(r), AccountKeys::Pair(AccountKeys::Secret(_s)).address()};
	m_cached[r] = _s;
	m_keys[r] = move(key);
	save();
	return r;
...
{
	h128 r = h128::random();
    EncryptedKey key{encrypt(_s, _pass), toUUID(r), AccountKeys::Pair(AccountKeys::Secret(_s)).address()};
